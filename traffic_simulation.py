...
# no method to spawn cars with arrival rate, do i manually check each boundary node once per tick
# in a helper method?
...
def serve_intersection(t: int, node: Node) -> int:
    ...
    green_directions = signal_phase(t)
...
def spawn_car(v: Node, u: Node, t: int):
    # if the random number generated is lower than the arrival rate,
    # a new car is spawned with its own id at the V -> U transit queue
    # intended only for boundary nodes
    if (random.random() < ARRIVAL_RATE):
        global car_id
        new_car = Car(car_id := car_id + 1, t)
        enqueue_departure(v, u, new_car)
...
def simulate_traffic():
    ...
    while (current_tick < TOTAL_TICKS):
        green_directions = signal_phase(current_tick)
        # once per tick per every node:
        # spawn cars if on boundary, move cars from transit to stop queue,
        # count # of cars in stop queue, then move cars
        for u in nodes:
            for v, direction in incoming_for(u):
                # if traffic isn't moving towards direction in current cycle,
                # count stopped traffic & increment queue_samples b/c traffic
                # moving in transit across intersctions is moving, not stopped
                # at any intersection. 
                if direction not in green_directions:
                    sum_queue += len(stopped[(v,u)]) 
                    queue_samples += 1
                # if v -> u is a boundary link,
                # spawn a car randomly
                if (is_boundary_incoming_link(v,u)):
                    spawn_car(v, u, current_tick)   
                # move a car from transit to stopped queue
                pop_to_queue_if_arrived(v, u)
            # cars in stopped queue move through/across intersection
            serve_intersection(current_tick, u)

            # after sampling and moving traffic at every single node once,
            # run next tick
        current_tick += 1

    # print report
    size_str = f"Size of grid: {N}x{N}\n"
    duration_str = f"Duration of simulation: {TOTAL_TICKS}\n"
    ew_light_str = f"E/W green light duration: {CYCLE_EW_GREEN} -- "
    ns_light_str = f"N/S green light duration: {CYCLE_NS_GREEN}  -- "
    total_light_str = f"Total green light duration: {CYCLE_NS_GREEN + CYCLE_EW_GREEN}\n"
    arrival_str = f"Arrival rate: {ARRIVAL_RATE}\n"
    trips_str = f"Total number of trips completed: {completed}\n"
    throughput_str = f"Throughput: {completed/TOTAL_TICKS}\n"
    travel_time_str = f"Mean travel time in the grid: {sum_tt/completed}\n"
    queued_vehicles_str = f"Mean number of queued vehicles: {sum_queue/max(1, queue_samples)}\n"
    
    print(size_str + duration_str + ew_light_str + ns_light_str + total_light_str + 
          arrival_str + trips_str + throughput_str + travel_time_str + queued_vehicles_str)
